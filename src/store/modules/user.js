...
function getMenusName() {
  const raw = isPlatform
    ...
    : [];
  // 菜单可见性以 eb_system_menu.is_show 为准，前端不再二次过滤
  return raw;
}
const state = {
  token: getToken(),
...
const actions = {
  ...
  getMenus({ commit }) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      // 菜单全部来自 eb_system_menu：审批中心 / 仓库管理 / 京东物流 / 推荐管理 /
      // 七件事装修 / 发货记录 / 企业四要素核验 / 字典列表 原来是在这里硬拼进来的，
      // 绕开了菜单表，导致「权限规则」里看不到、角色勾不上、调整一次要发一次前端包。
      // 现已迁到菜单表，见 sql/menu_migrate_hardcoded_routes.sql。
      // 菜单不显示先查两处：eb_system_menu.is_show，以及该菜单有没有授权给当前角色
      //（getMenusByUserId 是按 eb_system_role_menu 关联查的，没授权接口就不返回）。
