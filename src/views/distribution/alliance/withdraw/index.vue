  ...
  <div class="divBox">
    <el-card shadow="never">
      ...
      <el-table :data="list" border size="small" v-loading="loading">
        ...
        <el-table-column prop="applyAmount" label="提现金额" width="110" />
...
<script>
import { getLeaderWithdrawList, approveLeaderWithdraw, rejectLeaderWithdraw, markLeaderWithdrawPaid } from '@/api/alliance';

export default {
  name: 'AllianceLeaderWithdraw',
  data() { return { status: null, list: [], loading: false, saving: false, dialogVisible: false, dialogTitle: '', remark: '', current: null, action: '' }; },
  mounted() { this.load(); },
  methods: {
    ...
    load() { this.loading = true; getLeaderWithdrawList({ status: this.status, page: 1, size: 100 }).then((res) => { this.list = res.data || []; }).finally(() => { this.loading = false; }); },
