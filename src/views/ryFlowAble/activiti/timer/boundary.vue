  ...
  <div class="app-container">
    ...
    <el-dialog title="启动流程" :visible.sync="startOpen" width="500px" append-to-body>
      <el-form ref="startForm" :model="startForm" :rules="startRules" label-width="100px">
        ...
        <el-form-item label="菜品选择" prop="food">
             <el-select v-model="startForm.food" radio placeholder="请选择食物">
                <el-option v-for="item in foodOptions" :key="item.id" :label="item.name" :value="item.id"  ></el-option>
            </el-select>
        </el-form-item>
      </el-form>
...
<script>
import { boundary, pageQuery, boundaryTrigger } from "@/api/ryFlowAble/activiti/timer"
...
export default {
  ...
  data() {
    return {
      ...
      startRules: {
        food: [
          { required: true, message: "请选择食物", trigger: "blur" }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    handleTrigger(row){
        ...
        boundaryTrigger(params).then(res=>{
             this.$modal.msgSuccess("模拟用户选好菜品")
             this.resetQuery();
        })
    },
    /** 搜索按钮操作 */
    ...
    handleAdd() {
        this.startOpen = true;
        this.startForm.error = undefined;
    },
    /** 查看按钮操作 */
    handleView(row) {
      // 跳转到流程详情页面
