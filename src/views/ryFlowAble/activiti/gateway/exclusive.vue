    ...
    <el-dialog title="启动流程" :visible.sync="startOpen" width="500px" append-to-body>
      <el-form ref="startForm" :model="startForm" :rules="startRules" label-width="100px">
        <el-form-item label="模型Key">
          <el-input v-model="startForm.modelKey" disabled />
        </el-form-item>
        <el-form-item label="请假天数" prop="days">
             <el-select v-model="startForm.days" radio placeholder="请选择天数">
                <el-option v-for="item in daysOptions" :key="item.id" :label="item.name" :value="item.id"  ></el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="请假类型" >
          <el-input v-model="startForm.type" placeholder="请输入请假类型" />
        </el-form-item>
        <el-form-item label="请加理由">
          <el-input v-model="startForm.reason" type="textarea" placeholder="请输入请假理由" />
        </el-form-item>
      </el-form>
...
import { startExclusiveCase } from "@/api/ryFlowAble/activiti/gateway"
...
export default {
  ...
  data() {
    return {
      ...
      startRules: {
        days: [
          { required: true, message: "请加天数不能为空", trigger: "blur" }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    getList() {
      ...
      pageQuery(params).then(response => {
        this.processInstanceList = response.data.list || []
    ...
    handleAdd() {
        this.startOpen = true;
        this.startForm.days = undefined;
        this.startForm.type = undefined;
        this.startForm.reason = undefined;
    },
    /** 查看按钮操作 */
    handleView(row) {
      // 跳转到流程详情页面
      ...
      this.$refs["startForm"].validate(valid => {
        if (valid) {
          ...
          const variables = {
            ...
            days: this.startForm.days,
            reason: this.startForm.reason,
            type: this.startForm.type
          }
          ...
          startExclusiveCase(variables).then(response => {
