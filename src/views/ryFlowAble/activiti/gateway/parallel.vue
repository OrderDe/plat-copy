...
import { startParallel } from "@/api/ryFlowAble/activiti/gateway"
...
export default {
  ...
  data() {
    return {
      ...
      startRules: {
        name: [
          { required: true, message: "员工姓名", trigger: "change" }
        ]
      },
  ...
  methods: {
    handleUploadSuccess(response, file) {
      // 适配若依返回结构：{ code, msg, fileName, url }
      if (response && response.code === 200) {
        this.startForm.imageUrl = response.url || response.fileName || ''
        this.$message.success('上传成功')
      } else {
        this.$message.error(response && response.msg ? response.msg : '上传失败')
      }
    },
    beforeImageUpload(file) {
      ...
      if (!isLt5M) {
        this.$message.error('图片大小需小于 5MB')
      }
      ...
      this.$refs["startForm"].validate(valid => {
        if (valid) {
          ...
          const variables = {
            ...
            //实际业务使用上传后的地址
            imageUrl: this.startForm.imageUrl,
          }
          ...
          startParallel(variables).then(response => {
