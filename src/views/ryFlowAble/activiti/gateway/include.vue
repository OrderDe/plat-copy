    ...
    <el-dialog title="启动流程" :visible.sync="startOpen" width="500px" append-to-body>
      <el-form ref="startForm" :model="startForm" :rules="startRules" label-width="100px">
        ...
        <el-form-item label="员工类型" prop="userType">
             <el-select v-model="startForm.userType" radio placeholder="请选择员工类型">
                <el-option v-for="item in userTypeOptions" :key="item.id" :label="item.name" :value="item.id"  ></el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="员工姓名" >
          <el-input v-model="startForm.name" placeholder="请输入姓名" />
        </el-form-item>
      </el-form>
...
import { startInclude } from "@/api/ryFlowAble/activiti/gateway"
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
        this.startOpen = true;
        this.startForm.userType = undefined;
        this.startForm.name = undefined;
    },
    /** 查看按钮操作 */
    handleView(row) {
      // 跳转到流程详情页面
      ...
      this.$refs["startForm"].validate(valid => {
        if (valid) {
          ...
          const variables = {
            ...
            userType: this.startForm.userType,
          ...
          startInclude(variables).then(response => {
