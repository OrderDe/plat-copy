    ...
    <el-table v-loading="loading" :data="processInstanceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-top" @click="handleTrigger(scope.row)">触发信号</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="启动流程" :visible.sync="startOpen" width="500px" append-to-body>
      <el-form ref="startForm" :model="startForm" :rules="startRules" label-width="100px">
        ...
        <el-form-item label="投诉内容" >
          <el-input v-model="startForm.complain" placeholder="请输入投诉内容" />
        </el-form-item>
      </el-form>
...
import { startEventBase, triggerSignal } from "@/api/ryFlowAble/activiti/gateway"
...
export default {
  ...
  methods: {
    ...
    handleTrigger(row){
        ...
        triggerSignal(params).then(res=>{
             this.$modal.msgSuccess("触发了正式员工处理信号");
             this.resetQuery();
        })
    },
    /** 提交启动流程 */
      ...
      this.$refs["startForm"].validate(valid => {
        if (valid) {
          ...
          const variables = {
            ...
            complain: this.startForm.complain
          }
          ...
          startEventBase(variables).then(response => {
