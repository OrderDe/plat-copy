  ...
  <div class="app-container">
    ...
    <el-dialog title="启动流程" :visible.sync="startOpen" width="500px" append-to-body>
      <el-form ref="startForm" :model="startForm" :rules="startRules" label-width="100px">
        ...
        <el-form-item label="消息类型" prop="bookingType">
             <el-select v-model="startForm.bookingType" radio placeholder="消息类型">
                <el-option v-for="item in bookingTypeOptions" :key="item.id" :label="item.name" :value="item.id"  ></el-option>
            </el-select>
        </el-form-item>
      </el-form>
...
import { start } from "@/api/ryFlowAble/activiti/message"
...
export default {
  ...
  data() {
    return {
      ...
      // 消息类型
      bookingTypeOptions:[
        {id: "telMessage",name: "电话订票"},
        {id: "pcMessage",name: "网络订票"},
      ],
      // 列信息
      columns: [
      ...
      startRules: {
        bookingType: [
          { required: true, message: "订票类型不能为空", trigger: "blur" }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    handleAdd() {
        this.startOpen = true;
        this.startForm.bookingType = undefined;
    },
    /** 查看按钮操作 */
    handleView(row) {
      // 跳转到流程详情页面
      ...
      this.$refs["startForm"].validate(valid => {
        if (valid) {
          ...
          const variables = {
            ...
            bookingType: this.startForm.bookingType,
          }
