...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      product
        .guaranteePlatListApi(this.tableFrom)
        .then((res) => {
    ...
    handlerOpenDel(rowData) {
      this.$modalSure('删除当前保障服务吗').then(() => {
        product.guaranteePlatDeleteApi(rowData.id).then((data) => {
    ...
    onchangeIsShow(row) {
      product.guaranteePlatShowApi(row.id).then(() => {
        this.$message.success('操作成功');
        this.getList();
      });
    },
