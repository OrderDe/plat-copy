  ...
  <div class="divBox" style="padding-top: 0">
    <el-row :gutter="14">
      <el-col
        ...
        :md="24"
        :lg="12"
        v-if="checkPermi(['platform:statistics:home:operating:data', 'circle:statistics:home:operating'])"
      >
      ...
      <el-col
        ...
        :md="24"
        :lg="12"
        v-if="checkPermi(['platform:statistics:home:user:channel', 'circle:statistics:home:channel'])"
      >
        <el-card class="box-card" shadow="never" :bordered="false">
...
export default {
  ...
  mounted() {
    ...
    if (checkPermi(['platform:statistics:home:user:channel', 'circle:statistics:home:channel'])) this.getChannel();
    if (checkPermi(['platform:statistics:home:operating:data', 'circle:statistics:home:operating']))
    ...
    getbusinessData() {
      if (isPlatform) {
        businessData()
          ...
          .catch((error) => {
            console.error('Platform business data API error:', error);
          });
      } else {
        areaApi
          .areasNodeHomeOperatingDataApi()
          .then((res) => {
            this.statisticData[0].num = res.onSaleProductNum; //在售商品数量
            this.statisticData[1].num = res.awaitAuditProductNum; //待审核商品数量
            this.statisticData[2].num = res.notShippingOrderNum; //待发货订单数量
          ...
          .catch((error) => {
            console.error('Area business data API error:', error);
          });
      }
    },
    onchangeTime(e) {
    ...
    getChannel() {
      if (isPlatform) {
        userChannelData()
          ...
          .catch((error) => {
            console.error('Platform user channel data API error:', error);
          });
      } else {
        areaApi
          .areasNodeHomeChannelApi()
          .then((res) => {
            this.processChannelData(res);
          })
          .catch((error) => {
            console.error('Area user channel data API error:', error);
          });
      }
    },
    processChannelData(res) {
      // 处理不同API返回结构的差异
      ...
      this.optionData = {
        ...
        legend: {
          ...
          formatter: (name) => {
            ...
            percent = total > 0 ? this.$selfUtil.Division(num, total) : 0; //除法算出环比，避免除零
            let arr = [
