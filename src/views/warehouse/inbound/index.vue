<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tableData" border stripe>
      ...
      <el-table-column prop="otherBoundCode" label="关联单据" width="160" />
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px" size="small" :disabled="dialogMode === 'view'">
        <el-row :gutter="16">
          ...
          <el-col :span="12">
            <el-form-item label="关联单据">
              <el-input v-model="form.otherBoundCode" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-table :data="form.items" border style="margin-top:8px" size="mini">
          ...
          <el-table-column label="生产批次" width="130">
            <template slot-scope="{row}">
...
<script>
import { inboundApi } from '@/api/warehouse';
...
export default {
  ...
  methods: {
    emptyForm() { return { warehouseId: null, type: 0, otherBoundCode: '', applyUserName: '', remark: '', items: [] }; },
    ...
    addItem() {
      this.form.items.push({ productId: null, platformType: 0, goodsName: '', inboundTotalNum: 0, actualInboundNum: 0, productionDate: '' });
    },
    ...
    async onSubmitForm() {
      ...
      this.saving = true;
      try {
        await inboundApi.add(this.form);
